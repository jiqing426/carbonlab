import { AppTokensResponse, AppTokensQueryParams, AppSecretApiResponse } from '@/types/tale'
...
// 获取 App Token 列表
export async function getAppTokens(appKey: string, params?: AppTokensQueryParams): Promise<AppTokensResponse> {
  if (!appKey) {
  ...
  if (params?.is_valid !== undefined) queryParams.append('is_valid', params.is_valid.toString())
  if (params?.sort_by) queryParams.append('sort_by', params.sort_by)
  if (params?.sort_direction) queryParams.append('sort_direction', params.sort_direction)
  if (params?.search) queryParams.append('search', params.search)

  const response = await fetch(`${API_BASE_URL}/app/v1/app/tokens?${queryParams}`, {
    method: 'GET',
    headers: {
